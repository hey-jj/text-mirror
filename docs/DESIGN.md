...
- `detect`: format detection, magic bytes first with the extension as tiebreak, against a versioned format table. A table entry can also claim exact basenames, so a file named exactly `.env` declares its format without an extension. Declared and detected types are both recorded, with a mismatch flag when they disagree. A declared id the table marks as a refinement of what magic saw, svg over bare xml, resolves to the declared id with the flag down, since the two observations agree. Magic that resolves to a format outside the table cannot contradict the declared id, so it leaves the flag down and appends a warning naming what the bytes showed.
...
An external conversion provider is a second deployment-supplied runtime, and it is opt-in twice over: the feature must be built and a configuration must name every role. The configuration is deployment data and never versioned rules data, because it holds absolute paths. The expectations it pairs with them, a BLAKE3, an exact version, and an aggregate digest over the enumerated closure, every regular file under each entry and the target of every symlink, are re-asserted in the cold jailed worker before each execution, so a component swapped after any parent-side validation still fails closed. A closure is enumerated rather than rooted: each entry is granted its own subpath, a shared root is refused, and a role that enumerates a closure must have it pinned, so no grant reaches further than what a deployment measured and the rules assert. A provider runs under its own measured jail class, separate from every other, whose allowances key on the class together with the wired components rather than on either alone. The class is held to the same controls as the narrower ones: no egress, no read or write outside the per-run jail, no execution of an undeclared program, no inherited environment, no surviving process, and a fresh working root per execution, which is where a component's own profile state lives. A platform with no measured profile for the class refuses it, and never runs the components under a profile measured for something else. A configured provider that cannot run records a failed derived child with a reason naming a generic role label, never an unsupported record and never a silently missing child.

A configured provider is part of the effective rules version: `Rules::version()` appends a digest of what the configuration pins, so enabling, disabling, or repinning a provider re-runs the affected sources, while moving an identical installation to another path re-runs nothing. The digest excludes absolute paths for exactly that reason, and it includes the jail profile template's hash and the two jail parameters, so a run under different parameters or an edited template is a different version.
...
Every `unsupported` record names its reason in the error field. A declared entry in `rules/converters.toml` wins, and a detected format with no converter entry and no declared reason records the floor reason `no-converter`. The declared vocabulary: `engine-unpinned` for media formats awaiting a pinned engine, `hidden-visibility-unresolved` for spreadsheet formats with no visibility reader, `converter-deferred` for recognized data formats awaiting a converter, `container-deferred` for archive formats awaiting container expansion, `proprietary-binary` for design-tool binaries with no public text extraction worth building, `records-worker-not-built` for parquet, avro, and sqlite on a build compiled without the `records-worker` feature, and `pickle-deserialization-unsafe` for pickle files. The `records-worker-not-built` reason is set at registry construction: the converter exists and the rules route to it, but a binary built without the feature has no records mode in its worker, so the format is a deliberate capability gap that a build with `--features records-worker` closes. The `pickle-deserialization-unsafe` reason is a permanent security exclusion: deserializing a pickle stream executes arbitrary code, so never add a pickle deserializer to this tree and never parse pickle bytes. A script can bucket the whole unconverted remainder on these strings alone.
...
In-process coverage today: the text-native passthrough, a document adapter for Office documents, presentations, rich text, OpenDocument files, and EPUB, a workbook converter for xls, xlsx, xlsm, and ods, a streaming html strip, an eml converter, and zip container expansion. The html strip keeps the visible text with the title as the first line, drops script, style, template, noscript, and non-title head content, turns block boundaries into line breaks, decodes character references, and collapses whitespace, all without building a document tree, so memory stays bounded under hostile nesting. The eml converter renders a fixed header subset with encoded words decoded, then the message's text parts in MIME order, routing an html-only body through the strip path. Each attachment is decoded and lifted out as a container member, named by its 1-based index and sanitized filename, and routed back through dispatch under the message's `.d/` directory. Text-layer PDF runs through the subprocess runner, because a PDF parser over hostile bytes belongs in a jailed child that can crash without taking the pipeline down. A PDF with no extractable text layer fails with reason `pdf_no_text_layer`. Failed records re-run every pass, so those files convert automatically once an OCR converter lands. The passthrough accepts UTF-8, and UTF-16 behind a required byte order mark: the mark is consumed, the text transcodes, and the record carries a transcoding warning naming the source encoding. A UTF-16 file with no mark fails as `invalid_utf8`, and a UTF-32 mark fails with `unsupported-encoding: utf-32`, since a guessed encoding would emit decoded noise as a converted artifact. A NUL scalar anywhere in converter output fails with `nul_bytes` naming the first offset, enforced at the pipeline output gate over every converter's text, because no legitimate text carries one and the usual source is a BOM-less UTF-16 export that needs re-exporting as UTF-8.
...
Containers expand into a sibling directory. A zip yields a member-listing artifact, one line per member with its declared size, plus a child record per member under `update.zip.d/`, and each child carries a `parent_source` link. An eml yields its rendered headers and body plus a child per attachment, named by the 1-based attachment index and the sanitized filename. Members route back through the same dispatch, so a docx member converts through the document converter and a zip member expands again, and the child path scheme `a.zip.d/b.zip.d/c.txt` encodes the full chain with no new manifest field. Expansion runs in process, because a jailed child cannot recurse through the registry and dedup index, and the caps below neutralize a zip bomb the same way the workbook path already inflates OOXML archives. It runs under hard limits held as rules data: nesting depth, members per container, cumulative expanded bytes, and per-member size. Enforcement is two stage. The central directory is read first, so the child count and declared sizes fail the whole container before any inflation, then each member inflates through a capped reader that never trusts a declared size and charges the actual bytes against the cumulative budget. A member name that is absolute, rooted, drive-prefixed, escapes with `..`, carries a control character, or is a symlink is refused as `unsafe-member-path`, the raw name inspected before normalization, so an unsafe name is refused where the zip crate would silently rebase it into the tree. An encrypted member is refused as `encrypted`, a member repeating an ancestor container's hash as `container-recursive` which kills a zip quine before it spends the depth budget, and two members whose normalized paths would alias one artifact are refused as `member-path-collision`.

Container expansion is one operation whose completion is atomic in effect and resumable after a kill. The parent record is written last, so an intact parent artifact proves the whole expansion finished, and a crash before it re-does the container on the next run. On re-expansion the expander computes the new descendant set and retires every prior descendant the container no longer holds with a terminal `container-member-removed` record, removing its artifact, so no dropped member survives as a live converted record. A failure path retires the prior descendants the same way, and a container that checkpoint-skips first confirms every text-bearing descendant still hashes to its record, or re-expands, which gives a container subtree the self-healing a top-level file already has. The frozen `parent_source` field and the `.d/` path scheme carry this with no schema change.
...
Incremental and idempotent: the manifest is the checkpoint. The key is (source_path, source_hash, converter_version, rules_version), where the rules version is the effective one, which carries the configured provider's identity when there is one. A match means `skipped_unchanged` with no work done. Traversal is sorted, so a killed run resumes by re-walking and skipping completed records, with no separate journal to corrupt. Path plus size plus mtime is never sufficient to skip.

Dedup: an identical source hash converts once, but every duplicate still gets a real text file at its parallel path so the mirror stays complete, and the duplicate's `dedup_of` points at the canonical copy for corpus assembly. The duplicate also inherits the canonical's conversion warnings, since they describe the shared bytes, so a consumer slicing by warning sees every affected record without a join.
