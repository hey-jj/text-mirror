...
- `detect`: format detection, magic bytes first with the extension as tiebreak, against a versioned format table. Declared and detected types are both recorded, with a mismatch flag when they disagree.
...
One trait, two implementation styles. In-process converters are pure-Rust parsers for documents, spreadsheets, and text-layer PDF. Subprocess adapters wrap native tools and models for transcription, media decoding, and OCR. Both styles emit the same outcome: `{converter_id, converter_version, detected_format, text, warnings}`, with an optional media block for audio and video.
